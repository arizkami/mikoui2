...
impl RightPanel {
    ...
    pub fn resize_handle_rect(&self) -> Rect {
        Rect::from_xywh(
            self.x - RESIZE_HANDLE_WIDTH / 2.0,
            self.y,
            RESIZE_HANDLE_WIDTH,
            self.height,
        )
    ...
    pub fn resize_to(&mut self, x: f32, window_width: f32) {
        let new_width = (window_width - x).clamp(MIN_WIDTH, MAX_WIDTH);
